...
export default function TransactionFilter() {
  ...
  const selectedStatus = searchParams.get("status") || "";
  const selectedFromWalletSender = searchParams.get("fromWalletSender") || "";
  const selectedFromWalletEmail = searchParams.get("fromWalletEmail") || "";
  const selectedFromWalletPhone = searchParams.get("fromWalletPhone") || "";
  const selectedFromWalletRole = searchParams.get("fromWalletRole") || "";
  const selectedToWalletReceiver = searchParams.get("toWalletReceiver") || "";
  const selectedToWalletEmail = searchParams.get("toWalletEmail") || "";
  const selectedToWalletPhone = searchParams.get("toWalletPhone") || "";
  const selectedToWalletRole = searchParams.get("toWalletRole") || "";
  const searchTerm = searchParams.get("searchTerm") || "";
  const sort = searchParams.get("sort") || "-createdAt";
  ...
  const senderEmailOptions =
    allCategoriesOfUser?.data
      ?.filter(
        (item: any, index: any, array: any) =>
          array.findIndex((t: any) => t.email === item.email) === index &&
          item.email
      )
      .map((item: { email: string }) => ({
        label: item.email,
        value: item.email,
      })) || [];
  ...
  return (
    <div className="col-span-3 w-full space-y-4">
      ...
      <div>
        <Label className="lg:mb-3 mb-2">Search</Label>
        <Input
          placeholder="Search by name, phone, email, role, type, status..."
          value={searchTerm}
          onChange={(e) => handleFilterChange("searchTerm", e.target.value)}
        />
        {/* <p className="text-xs text-muted-foreground mt-1">
          Search across all fields: names, phones, emails, roles, types,
          statuses, amounts, etc.
        </p> */}
      </div>

      {/* Additional Filters (Collapsible for better UX) */}
      <div className="grid lg:grid-cols-2 gap-4">
        {/* Transaction Type Filter */}
        <div>
          <Label className="lg:mb-3 mb-2">Transaction Type</Label>
      ...
      {data?.data?.role === "ADMIN" && (
        <>
          <div className="grid lg:grid-cols-2 gap-4">
            ...
            {/* Sender Email Filter */}
            <div>
              <Label className="lg:mb-3 mb-2">Sender Email</Label>
              <Select
                value={selectedFromWalletEmail}
                onValueChange={(value) =>
                  handleFilterChange("fromWalletEmail", value)
                }
                disabled={allCategoriesOfUserLoading}
              >
                <SelectTrigger className="w-full">
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectLabel>Sender Name</SelectLabel>
                    {senderEmailOptions?.map((item: any) => (
                      <SelectItem key={item.value} value={item.value}>
                        {item.label}
                      </SelectItem>
                    ))}
            ...
            {/* Receiver Email Filter */}
            <div>
              <Label className="lg:mb-3 mb-2">Receiver Name</Label>
              <Select
                value={selectedToWalletEmail}
                onValueChange={(value) =>
                  handleFilterChange("toWalletEmail", value)
                }
                disabled={allCategoriesOfUserLoading}
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="Select receiver email" />
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    <SelectLabel>Receiver Email</SelectLabel>
                    {receiverEmailOptions?.map((item: any) => (
                      <SelectItem key={item.value} value={item.value}>
                        {item.label}
                      </SelectItem>
                    ))}
      ...
      {/* <div className="flex items-center justify-center gap-2 border-2 border-primary shadow rounded-xl px-3">
        ...
        >
          <SelectTrigger className="border-none shadow-none px-0">
            ...
            <SelectValue>{searchParams.limit} Per Page</SelectValue>
          </SelectTrigger>
          <SelectContent>
