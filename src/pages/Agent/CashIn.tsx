...
export default function CashIn() {
  ...
  const cashInSchema = z.object({
    ...
    amount: z
      ...
      .min(100, { message: "Minimum cash-in amount 100 TK" })
  ...
  const onSubmit = async (data: z.infer<typeof cashInSchema>) => {
    if (!selectedUser) {
      toast.error("Please select a user to cash-in to");
      return;
    }
    ...
    try {
      ...
      if (result?.success) {
        toast.success("Cash-In money successfully!", { id: toastId });
        form.reset();
  ...
  return (
    <div className="xl:mt-8 lg:mt-6 md:mt-4 mt-3 xl:mb-24 lg:mb-20 md:mb-16 mb-12 xl:px-20 lg:px-14 md:px-10 px-5">
      ...
      <div className="bg-accent/50 xl:mt-12 lg:mt-10 md:mt-8 mt-6 border-2 border-primary rounded-2xl xl:p-16 lg:p-10 md:p-8 p-5 lg:w-1/2 md:w-2/3 w-full mx-auto">
        ...
        {/* User Search */}
        <div className="mb-6">
          <label className="text-sm font-medium mb-2 block">Find User</label>
          <div className="relative border border-primary rounded-lg">
            <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
            <Input
              placeholder="Search by user phone or email..."
              value={searchTerm}
        ...
        <Form {...form}>
          ...
          >
            ...
            <Button
              ...
              disabled={cashInLoading || !selectedUser}
            >
              {cashInLoading ? (
                <>
                ...
                `Cash In to ${selectedUser?.name || "User"}`
              )}
            </Button>
          </form>
        </Form>
